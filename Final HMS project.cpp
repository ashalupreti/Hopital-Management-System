  ...
  class patientdetail
{
	...
	void patientsinfo()
	{
		...
		cout<<"\n\t\tPatient contact:";
			cin>>pcontact;
				 for(int i=0;pcontact[i];i++)   //validation for phone number
     {
     if((int(pcontact[i])<=48)&&int(pcontact[i]>=57)||int(pcontact[i])==45)
    {
    	cout<<endl<<Color(12)<<"INVALID FOUND!!! PLEASE INSERT NUMBERS ONLY.(eg.9818199024)"<<endl<<Color(8);
...
break;
				...
				cout<<"\n\t\tGender(M or Y?):";
				cin>>psex;
				if(int(psex[0]!=77||int(psex[0])!=70)||int(psex[0])!=102||int(psex[0])!=109)
{
	cin.get();
	...
	else
	{
		cout<<Color(13)<<"\n\t\tINVALID INPUT PLEASE ENTER THE VALID INPUT\n"<<Color(10);
				...
				cin>>pbldgp;
				string boj(pbldgp);
				if(boj!="ab+"||boj!="ab-"||boj!="a+"||boj!="a-"||boj!="b+"||boj!="b-"||boj!="0+"||boj!="o-")
				{
